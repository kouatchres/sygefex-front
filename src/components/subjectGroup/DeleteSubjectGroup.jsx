...
class deleteSubject extends Component {
	updateCache = (cache, payload) => {
		...
		const data = cache.readQuery({ query: getAllSubjectsQuery });
		// the deletedselect all the other regions except the deleted one from the cache
		data.divisions = data.divisions.filter((item) => item.id !== payload.data.deleteSubject.id);
		//  3. write the new data back to the cache
		console.log('getting payload');
		console.log(payload);
		cache.writeQuery({ query: getAllSubjectsQuery, data });
	};

	render() {
		return (
			<div>
				<Mutation mutation={deleteSubjectMutation} variables={{ id: this.props.id }} update={this.updateCache}>
					{(deleteSubject, { error }) => (
						<DelBtn
							onClick={() => {
