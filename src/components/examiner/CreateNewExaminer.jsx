...
const CreateNewExaminer = () => {
    ...
    const [createExaminer, { error }] = useMutation(createExaminerMutation)
    return (
        <Formik
            method="POST"
                                ...
                                <AllControls>
                                    <TwoGroups>
                                        ...
                                        <InputGroup>
                                            ...
                                            <SygefexSelect name="gender" onChange={value => setFieldValue('gender', value)} disabled={isSubmitting} placeholder={"Votre Sexe"} options={getGenderOptions} />

                                            <SygexInput name="examiner1stName" type="text" label="Nom" disabled={isSubmitting} />
                                            <SygexInput name="examiner2ndName" type="text" label="Prénom" disabled={isSubmitting} />
                                            <SygexInput name="examinerOtherNames" type="text" label="Autres Noms" disabled={isSubmitting} />
                                            <SygexInput name="examinerMatricule" type="text" label="Numéro Matricule" disabled={isSubmitting} />
                                            <SygexInput name="examinerCNI" type="text" label="Numéro de Carte d'Identité" disabled={isSubmitting} />
                                            <SygexInput name="examinerPhone" type="number" label="Numéro de portable" disabled={isSubmitting} />
                                            <SygexInput name="examinerEmail" type="email" label="Votre Email" disabled={isSubmitting} />
                                            <PicFrame> {state.examinerImage && <img src={state.examinerImage} alt="Upload image" />}</PicFrame>
                                        </InputGroup>
                                    </TwoGroups>
                                    <ButtonStyled>
                                        <StyledButton disabled={isSubmitting} type="submit">Valid{isSubmitting ? 'ation en cours' : 'er'}</StyledButton>
                                    </ButtonStyled>
                                </AllControls>
                            </Form>
                        </StyledForm>
