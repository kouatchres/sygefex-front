...
const RankToModifyHook = () => {
  ...
  const { data: dataRank, loading: loadingRank, error: errorRank } = useQuery(
    getAllRanksQuery
  );
  {
  ...
  const rankOptions =
    getRank &&
    getRank.map((item) => ({ value: item.id, label: item.rankName }));
  console.log(getRank);
            ...
            <Form>
              <AllControls>
                <TwoGroups>
                  ...
                  <Buttons>
                    <ButtonStyled>
                      <StyledButton type="submit" id={state.rankID}>
                        Supprimer
                      </StyledButton>
                    </ButtonStyled>
                    <ButtonStyled>
